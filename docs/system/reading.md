...
A **section** is one `[data-segment]` element inside a feed item: a paragraph, a list, a code block, a diagram, the card's takeaway or hierarchy panel, or — on a lab card — its requirements, caution, cost and task panels. Markdown elements are tagged by `STATIC_COMPONENTS`; code blocks and diagrams tag themselves, including the diagram's loading skeleton — an async render that appeared late would otherwise shift every index below it.
...
**`data-item-id` sits on a wrapper, not on the card.** A card with a takeaway renders as two elements — the card and the panel under it — and both are sections of the same feed item, so the id that addresses them has to be above both. This is what a panel under a card costs: nothing. Every panel added since (the hierarchy, and lab mode's four) became readable, questionable and note-able by being a `[data-segment]` inside that wrapper.

**Not everything in the feed is a card.** The app speaks there too: a `notice` (the structured machine panel a lab lesson opens with, re-derived on every resume and never saved) and a `notification` (one line about the lesson's own state — a practice item added, a folder made — which happened once and is saved with the lesson). Neither carries `[data-segment]`, so the reading keys step straight past them; they are chrome in the column, not passages to read.

**The last section is where ArrowDown becomes the primary action.** On an ordinary card that is the takeaway, and the action is Continue; on a lab card it is the task panel, and the action is *I did it* — `advanceLesson` in `App.tsx` decides which, so the key and the button in the bar can never disagree (see `lab-mode.md`). See `takeaways.md` for why the panel itself carries `[data-segment]` rather than the paragraph inside it.
...
**The pane slides left for them.** The column and its margin are centred together as one block (`.reading-column`, driven by `data-margin` on the shell), so the lesson keeps its full reading width and moves left instead of being squeezed — and the panels and key bar move with it, which is how the app keeps one left edge (the top bar stays put; it is chrome, see `lesson-chrome.md`). Only a window too narrow to hold both starts taking width off the passage, and below `--reading-min` the margin is clipped instead. The scroll pane sets `overflow-x: clip` so that clipping never turns into a sideways scrollbar. See `questions.md`.
