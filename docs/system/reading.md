...
**The pane slides left for them.** The column and its margin are centred together as one block (`.reading-column`, driven by `data-margin` on the shell), so the lesson keeps its full reading width and moves left instead of being squeezed — and the header and key bar move with it, which is how the app keeps one left edge. Only a window too narrow to hold both starts taking width off the passage, and below `--reading-min` the margin is clipped instead. The scroll pane sets `overflow-x: clip` so that clipping never turns into a sideways scrollbar. See `questions.md`.
