...
- **Runtime per lesson.** How a turn is spawned — system prompt, tools, working directory, timeout — comes from a `TutorRuntime`, built once when the lesson starts from its mode (`src/bun/lesson-modes.ts`, and see `lesson-modes.md`). `claude.ts` runs turns without knowing what kinds of lesson exist. A lesson taught from a project gets read-only tools and runs in that project's folder; a topic lesson gets `--tools ""` as before; a lab lesson gets `WebSearch,WebFetch` and nothing that touches this machine — its steps are the learner's to run (see `lab-mode.md`). The **first** turn of any lesson runs on `openingRuntimeFor`, which is the same runtime with no tools: it asks the level question, and everything it could look up is already in its opening message.
...
Nothing is left to the machine's default tier. A lesson turn runs on the model and the effort **Settings** says, and every side-call is pinned in code.

| Call | Model | Thinking | Why |
|------|-------|----------|-----|
| Any lesson turn, any mode | Settings — `claude-opus-5` by default | Settings — `xhigh` by default | Teaching is the same job in all three modes, and what it costs is the learner's call |
| Any lesson's first turn | as above, minus every tool | as above | Asks the level question; the app already gave it the context |
...
| Lab check verdict | `claude-haiku-4-5-20251001` | **off** | Handed the expected output and the real one; a judgement, not a search |

Most of the side-calls are judgement-light by construction: grading is handed the expected answer rather than working the problem out, a lab check verdict is handed both the expectation and the real output, Mermaid repair is handed the parser error, and explanation is a lookup in a context that is already on screen. The teaching judgement lives in the lesson turn.
...
**One pair, for every mode and every turn.** `MODELS` and `EFFORTS` in `src/bun/settings.ts` are the menus; the webview draws them under **Tutor** in Settings and pushes the choice over `setSettings`. `runtimeFor` reads it when it builds a turn, so a change takes effect on the next card rather than the next lesson. The Bun side checks both values against its own lists before spawning anything — they end up on a command line, and the webview's localStorage is not the authority on what this app may run.

The mode spec no longer carries a model at all. Codebase and lab used to pin `claude-sonnet-5` and topic rode whatever the machine defaulted to, which meant the same app taught three lessons at three tiers and one of them depended on whose machine it was. Defaults are Opus 5 at `xhigh` — Opus because topic lessons were already getting it from the machine default, and anything cheaper would have made every existing lesson quietly worse the day the setting shipped.

**The scale starts at `high`, and `low` is not on it.** This is measured, not cautious. Same prompt three ways: no flag spent 48 thinking tokens, `xhigh` spent 23, `low` spent **zero**. `low` does not think less — it stops thinking. A lab lesson run with `teach: low` produced a card that taught "image" and "container" in two sentences, breaking the one-new-term rule in `prompts/tutor.md`, and a setup task for Docker on a machine the probe had already reported Docker running on. `medium` is left off the menu for the same reason: nobody has measured where thinking actually stops, so the floor sits where it is known to be safe.
...
1. **Tolerant parse** — `parseReply` retries `JSON.parse` after re-escaping raw control characters inside string literals (`escapeControlCharsInStrings`). An unescaped newline in the body is the most common break, and this recovers the whole card — outline and exercise included — with no model round-trip. It cannot fix an unescaped quote or a truncated reply, which fall through.
